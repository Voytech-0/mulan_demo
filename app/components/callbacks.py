...
from .settings import get_image_style, get_no_image_message_style, get_generative_placeholder_style, CELL_STYLE, \
    CELL_STYLE_RIGHT, TABLE_STYLE, EMPTY_METADATA_STYLE, get_no_metadata_message_style, BACKGROUND_COLOR
...
def create_metadata_display(dataset_name, data):
    ...
    metadatabox = html.Div([
                    html.P(f"Number of samples: {data.data.shape[0]}", style={'marginBottom': '0.8rem', 'lineHeight': 1.2, 'text-align': 'left'}),
                    html.P(f"Number of features: {data.data.shape[1]}", style={'marginBottom': '0.8rem', 'lineHeight': 1.2, 'text-align': 'left'}),
                    html.P(f"Number of classes: {len(np.unique(data.target))}", style={'marginBottom': '0.8rem', 'lineHeight': 1.2, 'text-align': 'left'})
                ])
...
def register_callbacks(app):
    ...
    def update_graphs(dataset_name, recalculate_flag, show_images, trimap_n_clicks, tsne_n_clicks, umap_n_clicks, cached_embeddings,
        ...
        # Metadata display
        metadata, fig = create_metadata_display(dataset_name, data)
